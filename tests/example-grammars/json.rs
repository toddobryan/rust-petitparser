...
use rust_petitparser::parser::character::character::{char, char_ci, digit, digit_with_radix, none_of, one_of};
...
fn raw_string_parser() -> impl Parser<String> {
    let unicode_escape = seq2(
        char('u'),
        digit_with_radix(16).times(4).input(),
    ...
    seq3(char('"'), string_char.star(), char('"'))
        .map(|(_, chars, _)| chars.into_iter().collect())
}
...
pub fn json_parser() -> impl Parser<Json> {
    ...
    let object = seq3(
        char('{'),
        member.star_sep(char(',').trim()),
        char('}'),
    ).map(|(_, ms, _)| Json::Object(ms));

    json_value.set(
        choice6(null, boolean, num, json_string, array, object).trim(),
    );
...
fn json_object_test() {
    ...
    let success = json_parser().parse(json_str).unwrap();
    assert_that!(success.value, eq(&Json::Object(vec![
        ("name".to_string(), Json::Str("John".to_string())),
        ("age".to_string(), Json::Num(30.0)),
        ("city".to_string(), Json::Str("New York".to_string())),
    ])));
    assert_that!(success.context.position, eq(47));
}
...
fn array_test() {
    ...
    assert_that!(success.context.position, eq(50));
}