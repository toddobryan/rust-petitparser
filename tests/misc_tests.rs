use googletest::prelude::*;
use rust_petitparser::prelude::*;
use rust_petitparser::{assert_failure, assert_success};

#[gtest]
fn eof_on_empty_input() {
    assert_success!(eof(), "", (), 0);
}

#[gtest]
fn eof_at_end_of_input() {
    let p = seq2(char('a'), eof());
    assert_success!(p, "a", ('a', ()), 1);
}

#[gtest]
fn eof_fails_mid_input() {
    let p = eof();
    let failure = p.parse("a").unwrap_err();
    assert_that!(failure.context.position, eq(0));
}

#[gtest]
fn string_matches_literal() {
    let p = string("hello");
    assert_success!(p, "hello world", "hello", 5);
}

#[gtest]
fn string_fails_on_mismatch() {
    let p = string("hello");
    assert!(p.parse("world").is_err());
}

#[gtest]
fn string_mismatch_reports_message() {
    // same length, wrong content -> the literal-expected message (not the short-input one)
    assert_failure!(string("foo"), "bar", "Expected string: \"foo\"", 0);
}

// string_ignore_case

#[gtest]
fn string_ignore_case_matches_same_case() {
    assert_success!(string_ignore_case("foo"), "foo!", "foo", 3);
}

#[gtest]
fn string_ignore_case_matches_other_case() {
    // returns the actual matched substring, preserving the input's case
    assert_success!(string_ignore_case("foo"), "FOO!", "FOO", 3);
    assert_success!(string_ignore_case("foo"), "fOo!", "fOo", 3);
}

#[gtest]
fn string_ignore_case_fails_on_mismatch() {
    assert_failure!(
        string_ignore_case("foo"),
        "bar",
        "Expected string (case-insensitive): \"foo\"",
        0
    );
}

#[gtest]
fn string_fails_on_short_input() {
    let p = string("hello");
    assert!(p.parse("hi").is_err());
}

#[gtest]
fn string_followed_by_eof() {
    let p = seq2(string("ok"), eof());
    assert!(p.parse("ok").is_ok());
    assert!(p.parse("ok!").is_err());
}

#[gtest]
fn string_unicode() {
    // "é" is 2 bytes in UTF-8 but 1 char — length must be in chars, not bytes
    let p = string("héllo");
    assert_success!(p, "héllo world", "héllo", 5);
}

#[gtest]
fn string_emoji() {
    // 🎉 is 4 bytes in UTF-8 but 1 char
    let p = string("🎉🎂");
    assert_success!(p, "🎉🎂 party", "🎉🎂", 2);
}

// labeled

#[gtest]
fn labeled_passes_through_on_success() {
    let p = char('a').labeled("an 'a'");
    assert_success!(p, "abc", 'a', 1);
}

#[gtest]
fn labeled_replaces_message_on_wrong_char() {
    let p = char('a').labeled("an 'a'");
    assert_failure!(p, "bc", "an 'a'", 0);
}

#[gtest]
fn labeled_replaces_message_at_end_of_input() {
    let p = char('a').labeled("an 'a'");
    assert_failure!(p, "", "an 'a'", 0);
}

#[gtest]
fn labeled_works_on_compound_parser() {
    let p = string("hello").labeled("greeting");
    assert_failure!(p, "world", "greeting", 0);
}

// epsilon

#[gtest]
fn epsilon_succeeds_on_nonempty_input() {
    assert_success!(epsilon(), "abc", (), 0);
}

#[gtest]
fn epsilon_succeeds_on_empty_input() {
    assert_success!(epsilon(), "", (), 0);
}

#[gtest]
fn epsilon_does_not_consume_input() {
    let p = seq2(epsilon(), char('a'));
    assert_success!(p, "abc", ((), 'a'), 1);
}

#[gtest]
fn epsilon_with_returns_value() {
    assert_success!(epsilon_with(42), "abc", 42, 0);
}

#[gtest]
fn epsilon_with_does_not_consume_input() {
    let p = seq2(epsilon_with("ok"), char('x'));
    assert_success!(p, "xyz", ("ok", 'x'), 1);
}

// success

#[gtest]
fn success_returns_value_without_consuming() {
    assert_success!(success(99i32), "abc", 99, 0);
}

#[gtest]
fn success_works_on_empty_input() {
    assert_success!(success("done"), "", "done", 0);
}

#[gtest]
fn success_as_choice_fallback() {
    // success(-1) acts as a default when char('a') fails
    let p = choice2(char('a').map(|_| 1i32), success(-1i32));
    assert_success!(p, "abc", 1, 1);
    assert_success!(p, "xyz", -1, 0);
}

// failure

#[gtest]
fn failure_always_fails() {
    assert_failure!(failure(), "abc", "unable to parse", 0);
}

#[gtest]
fn failure_fails_on_empty_input() {
    assert_failure!(failure(), "", "unable to parse", 0);
}

#[gtest]
fn failure_with_message_uses_given_message() {
    assert_failure!(
        failure_with_message("no match here".to_string()),
        "abc",
        "no match here",
        0
    );
}

#[gtest]
fn failure_with_message_preserves_position() {
    let p = seq2(char('a'), failure_with_message("stop".to_string()));
    assert_failure!(p, "abc", "stop", 1);
}

#[gtest]
fn line_and_column_of_single_line() {
    let b = "abc";
    assert_that!(
        line_and_column_of(&Context::new(b, 0)),
        eq(&TextLocation { line: 1, column: 1 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 1)),
        eq(&TextLocation { line: 1, column: 2 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 2)),
        eq(&TextLocation { line: 1, column: 3 })
    );
}

#[gtest]
fn line_and_column_of_empty_buffer() {
    let b = "";
    assert_that!(
        line_and_column_of(&Context::new(b, 0)),
        eq(&TextLocation { line: 1, column: 1 })
    );
}

#[gtest]
fn line_and_column_of_lf_newline() {
    let b = "ab\ncd";
    assert_that!(
        line_and_column_of(&Context::new(b, 0)),
        eq(&TextLocation { line: 1, column: 1 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 1)),
        eq(&TextLocation { line: 1, column: 2 })
    );
    // position on '\n' itself is still treated as line 1
    assert_that!(
        line_and_column_of(&Context::new(b, 2)),
        eq(&TextLocation { line: 1, column: 3 })
    );
    // first character after newline
    assert_that!(
        line_and_column_of(&Context::new(b, 3)),
        eq(&TextLocation { line: 2, column: 1 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 4)),
        eq(&TextLocation { line: 2, column: 2 })
    );
}

#[gtest]
fn line_and_column_of_crlf_newline() {
    let b = "a\r\nb";
    assert_that!(
        line_and_column_of(&Context::new(b, 0)),
        eq(&TextLocation { line: 1, column: 1 })
    );
    // position on '\r' — still line 1
    assert_that!(
        line_and_column_of(&Context::new(b, 1)),
        eq(&TextLocation { line: 1, column: 2 })
    );
    // position on '\n' inside the \r\n token — still line 1
    assert_that!(
        line_and_column_of(&Context::new(b, 2)),
        eq(&TextLocation { line: 1, column: 3 })
    );
    // first character after \r\n
    assert_that!(
        line_and_column_of(&Context::new(b, 3)),
        eq(&TextLocation { line: 2, column: 1 })
    );
}

#[gtest]
fn line_and_column_of_cr_newline() {
    let b = "a\rb";
    assert_that!(
        line_and_column_of(&Context::new(b, 0)),
        eq(&TextLocation { line: 1, column: 1 })
    );
    // position on '\r' — still line 1
    assert_that!(
        line_and_column_of(&Context::new(b, 1)),
        eq(&TextLocation { line: 1, column: 2 })
    );
    // first character after \r
    assert_that!(
        line_and_column_of(&Context::new(b, 2)),
        eq(&TextLocation { line: 2, column: 1 })
    );
}

#[gtest]
fn line_and_column_of_multiple_lines() {
    let b = "a\nb\nc";
    assert_that!(
        line_and_column_of(&Context::new(b, 0)),
        eq(&TextLocation { line: 1, column: 1 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 1)),
        eq(&TextLocation { line: 1, column: 2 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 2)),
        eq(&TextLocation { line: 2, column: 1 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 3)),
        eq(&TextLocation { line: 2, column: 2 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 4)),
        eq(&TextLocation { line: 3, column: 1 })
    );
}

#[gtest]
fn line_and_column_of_position_past_end() {
    let b = "ab\ncd";
    // position equal to buffer length: column should be (len - last_offset + 1)
    assert_that!(
        line_and_column_of(&Context::new(b, 5)),
        eq(&TextLocation { line: 2, column: 3 })
    );
}

#[gtest]
fn line_and_column_of_blank_line() {
    // two newlines in a row create an empty middle line
    let b = "a\n\nb";
    assert_that!(
        line_and_column_of(&Context::new(b, 0)),
        eq(&TextLocation { line: 1, column: 1 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 2)),
        eq(&TextLocation { line: 2, column: 1 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 3)),
        eq(&TextLocation { line: 3, column: 1 })
    );
}

#[gtest]
fn line_and_column_of_mixed_line_endings() {
    // \n then \r then \r\n
    let b = "a\nb\rc\r\nd";
    assert_that!(
        line_and_column_of(&Context::new(b, 0)),
        eq(&TextLocation { line: 1, column: 1 })
    );
    assert_that!(
        line_and_column_of(&Context::new(b, 2)),
        eq(&TextLocation { line: 2, column: 1 })
    ); // 'b'
    assert_that!(
        line_and_column_of(&Context::new(b, 4)),
        eq(&TextLocation { line: 3, column: 1 })
    ); // 'c'
    assert_that!(
        line_and_column_of(&Context::new(b, 7)),
        eq(&TextLocation { line: 4, column: 1 })
    ); // 'd'
}

// newline

#[gtest]
fn newline_matches_lf() {
    assert_success!(newline(), "\n", &"\n".to_string(), 1);
}

#[gtest]
fn newline_matches_crlf() {
    assert_success!(newline(), "\r\n", &"\r\n".to_string(), 2);
}

#[gtest]
fn newline_matches_bare_cr() {
    assert_success!(newline(), "\r", &"\r".to_string(), 1);
}

#[gtest]
fn newline_fails_on_empty_input() {
    assert_failure!(newline(), "", "newline expected", 0);
}

#[gtest]
fn newline_fails_on_non_newline() {
    assert_failure!(newline(), "\x0c", "newline expected", 0);
}

// position

#[gtest]
fn position_at_start_of_empty_input() {
    let p = seq!(any().star(), position() => |_, pos| pos);
    assert_success!(p, "", 0, 0);
}

#[gtest]
fn position_after_consuming_one_char() {
    let p = seq!(any().star(), position() => |_, pos| pos);
    assert_success!(p, "a", 1, 1);
}

#[gtest]
fn position_after_consuming_two_chars() {
    let p = seq!(any().star(), position() => |_, pos| pos);
    assert_success!(p, "aa", 2, 2);
}

#[gtest]
fn position_after_consuming_three_chars() {
    let p = seq!(any().star(), position() => |_, pos| pos);
    assert_success!(p, "aaa", 3, 3);
}

#[gtest]
fn position_does_not_consume_input() {
    let p = seq2(position(), char('a'));
    assert_success!(p, "a", (0, 'a'), 1);
}
